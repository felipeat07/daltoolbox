...
do_fit.ts_tmlp <- function(obj, x, y) {
  if (is.null(obj$model)) 
    obj$model <- create_torch_mlp(obj$input_size, obj$hidden_size)
  
  df_train <- as.data.frame(x)
  df_train$t0 <- as.vector(y)
  
  obj$model <- train_torch_mlp(obj$model, df_train, obj$epochs, obj$deep_debug, obj$reproduce)
...
do_predict.ts_tmlp <- function(obj, x) {
  ...
  X_values$t0 <- 0
  prediction <- predict_torch_mlp(obj$model, X_values)
